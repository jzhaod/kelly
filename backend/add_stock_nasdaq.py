"""
Command-line utility to add a new stock to stock_settings.json with real data from NASDAQ.
"""
import json
import os
import sys
...
def add_stock(symbol):
    """
    Add a new stock to stock_settings.json with real volatility and expected return data from NASDAQ.
    ...
    # Calculate volatility and expected returns from NASDAQ data
    print(f"Fetching NASDAQ data for {symbol}...")
    stock_data = get_stock_volatility_and_returns([symbol], period="y5")
    ...
    if symbol in stock_data and stock_data[symbol]['volatility'] is not None and stock_data[symbol]['expectedReturn'] is not None:
        ...
        print(f"Failed to fetch NASDAQ data for {symbol}")
        return False

if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python add_stock_nasdaq.py SYMBOL")
        sys.exit(1)
